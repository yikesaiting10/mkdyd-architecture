...
#ifndef __MKD_SOCKET_H__
#define __MKD_SOCKET_H__
...
//一些宏定义放在这里-----------------------------------------------------------
#define MKD_LISTEN_BACKLOG  511    //已完成连接队列
#define MKD_MAX_EVENTS      512    //epoll_wait一次最多接收这么多个事件

typedef struct mkd_listening_s   mkd_listening_t, *lpmkd_listening_t;
typedef struct mkd_connection_s  mkd_connection_t,*lpmkd_connection_t;
typedef class  CSocekt           CSocekt;

typedef void (CSocekt::*mkd_event_handler_pt)(lpmkd_connection_t c); //定义成员函数指针

//--------------------------------------------
//一些专用结构定义放在这里，暂时不考虑放mkd_global.h里了
struct mkd_listening_s  //和监听端口有关的结构
{
	int                       port;        //监听的端口号
	int                       fd;          //套接字句柄socket
	lpmkd_connection_t        connection;  //连接池中的一个连接，注意这是个指针 
};

//该结构表示一个TCP连接
struct mkd_connection_s
{		
	mkd_connection_s();                                      //构造函数
	virtual ~mkd_connection_s();                             //析构函数
	void GetOneToUse();                                      //分配连接
	void PutOneToFree();                                     //回收连接


	int                       fd;                            //套接字句柄socket
	lpmkd_listening_t         listening;                     //如果这个链接被分配给了一个监听套接字，那么这个里边就指向监听套接字对应的那个lpmkd_listening_t的内存首地址		

	//------------------------------------	
	//unsigned                  instance:1;                    //【位域】失效标志位：0：有效，1：失效  
	uint64_t                  iCurrsequence;                 //引入的一个序号，每次分配出去时+1
	struct sockaddr           s_sockaddr;                    //保存对方地址

	mkd_event_handler_pt      rhandler;                       //读事件的相关处理方法
	mkd_event_handler_pt      whandler;                       //写事件的相关处理方法
	...
	char                      *psendMemPointer;               //发送完成后释放用的，整个数据的头指针，消息头 + 包头 + 包体
	...
	//--------------------------------------------------
	lpmkd_connection_t        next;                           //指向下一个本类型对象，用于把空闲的连接池对象串起来构成一个单向链表，方便取用
};

//消息头
typedef struct _STRUC_MSG_HEADER
{
	lpmkd_connection_t pConn;         //记录对应的链接
...
public:	
	int  mkd_epoll_init();                                                //epoll功能初始化	
	//int  mkd_epoll_add_event(int fd,int readevent,int writeevent,uint32_t otherflag,uint32_t eventtype,lpmkd_connection_t pConn);     
	                                                                      //epoll增加事件
	int  mkd_epoll_process_events(int timer);                             //epoll等待接收和处理事件

	int mkd_epoll_oper_event(int fd,uint32_t eventtype,uint32_t flag,int bcaction,lpmkd_connection_t pConn); 
	                                                                      //epoll操作事件
	
protected:
	//数据发送相关
	void msgSend(char *psendbuf);                                         //把数据扔到待发送对列中 
	void zdClosesocketProc(lpmkd_connection_t p_Conn);                    //主动关闭一个连接时的要做些善后的处理函数	
	
private:	
	void ReadConf();                                                      //专门用于读各种配置项	
	bool mkd_open_listening_sockets();                                    //监听必须的端口【支持多个端口】
	void mkd_close_listening_sockets();                                   //关闭监听套接字
	bool setnonblocking(int sockfd);                                      //设置非阻塞套接字	

	//一些业务处理函数handler
	void mkd_event_accept(lpmkd_connection_t oldc);                       //建立新连接
	void mkd_read_request_handler(lpmkd_connection_t pConn);              //设置数据来时的读处理函数
	void mkd_write_request_handler(lpmkd_connection_t pConn);             //设置数据发送时的写处理函数
	void mkd_close_connection(lpmkd_connection_t pConn);                  //通用连接关闭函数，资源用这个函数释放

	ssize_t recvproc(lpmkd_connection_t pConn,char *buff,ssize_t buflen); //接收从客户端来的数据专用函数
	void mkd_wait_request_handler_proc_p1(lpmkd_connection_t pConn);      //包头收完整后的处理	                                                                   
	void mkd_wait_request_handler_proc_plast(lpmkd_connection_t pConn);   //收到一个完整包后的处理，放到一个函数中，方便调用	
	void clearMsgSendQueue();                                             //处理发送消息队列  

	ssize_t sendproc(lpmkd_connection_t c,char *buff,ssize_t size);       //将数据发送到客户端 

	//获取对端信息相关                                              
	size_t mkd_sock_ntop(struct sockaddr *sa,int port,u_char *text,size_t len);  //根据参数1给定的信息，获取地址端口字符串，返回这个字符串的长度

	//连接池 或 连接 相关
	void initconnection();                                                //初始化连接池
	void clearconnection();                                               //回收连接池
	lpmkd_connection_t mkd_get_connection(int isock);                     //从连接池中获取一个空闲连接
	void mkd_free_connection(lpmkd_connection_t pConn);                   //归还参数pConn所代表的连接到到连接池中	
	void inRecyConnectQueue(lpmkd_connection_t pConn);                    //将要回收的连接放到一个队列中来
	
	//和时间相关的函数
	void    AddToTimerQueue(lpmkd_connection_t pConn);                    //设置踢出时钟(向map表中增加内容)
	...
	void DeleteFromTimerQueue(lpmkd_connection_t pConn);                  //把指定用户tcp连接从timer表中抠出去
...
private:
	...
	//和连接池有关的
	std::list<lpmkd_connection_t>  m_connectionList;                      //连接列表【连接池】
	std::list<lpmkd_connection_t>  m_freeconnectionList;                  //空闲连接列表【这里边装的全是空闲的连接】
	...
	std::list<lpmkd_connection_t>  m_recyconnectionList;                  //将要释放的连接放这里
	std::atomic<int>               m_totol_recyconnection_n;              //待释放连接队列大小
	int                            m_RecyConnectionWaitTime;              //等待这么些秒后才回收连接	
	
	std::vector<lpmkd_listening_t> m_ListenSocketList;                    //监听套接字队列
	struct epoll_event             m_events[MKD_MAX_EVENTS];              //用于在epoll_wait()中承载返回的所发生的事件
